...
Не всё, что присылает поставщик, можно публиковать. 23.09.2026 поставщик
предупредил, что покупатели получают досудебные претензии из-за фотографии
товара **EV_a044057** «Гирлянда Eurosvet занавес 2*3м IP65 200-101 белый», и
попросил снять снимок отовсюду. К нам он попал через фид Декомо, как и все
остальные. Оригинал на dekomo.ru уже отдаёт 404 — поставщик его убрал.

Просто удалить нашу копию бесполезно: следующий импорт вернёт её из фида.
Поэтому блокировка хранится в таблице `blocked_content`, а импорт её соблюдает —
строка загружается с ценой и остатком, но без спорного содержимого.

```bash
python -c "from staging.db import db_session; from staging.blocked import purge;            conn=db_session().__enter__(); print(purge(conn))"
```

`scope = images` — товар грузится без картинок, `scope = product` — не грузится
вовсе. Снимается блокировка только руками: претензию отзывает человек, а не фид.

**На сайте этого мало.** Товар там скрыт (`status = 0`), но файл картинки
по-прежнему отдаётся публично — `/image/catalog/93899-1.jpeg`, 200 OK. Скрытый
товар не значит убранная фотография. Файлы нужно удалять с сервера, вместе с
уменьшенными копиями в `image/cache/`.
